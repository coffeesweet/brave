...
/**
 * Intercepts a client request and takes care of tracing the request. It will decide if we need to trace current request
 * and if so CS annotation will be submitted. You will also have to use {@link ClientResponseInterceptor} which will set
 * CR annotation.
 *
 * <p/>Abstraction on top of {@link ClientTracer}. If you want to integrate a library with brave it is advised to use
 * this abstraction instead of ClientTracer directly.</p>
 *
...
public class ClientRequestInterceptor {
    ...
    public void handle(final ClientRequestAdapter clientRequestAdapter, final Optional<String> serviceNameOverride) {
        ...
        clientTracer.submitBinaryAnnotation(REQUEST_ANNOTATION,
            clientRequestAdapter.getMethod() + " " + clientRequestAdapter.getUri());
    ...
    /**
     * Additional handling of Client Request. Subclasses can override this to hook in. Basic implementation does
     * nothing.
     *
